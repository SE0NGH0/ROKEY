...
def generate_launch_description():
    ...
    robots = [
        {'name': 'tb1', 'x_pose': '7.098', 'y_pose': '7.905', 'z_pose': '0.01','Yaw': '3.14159','model': 'waffle'},
        {'name': 'tb2', 'x_pose': '-6.949', 'y_pose': '7.036', 'z_pose': '0.01','Yaw': '0.0','model': 'waffle'},
        #{'name': 'tb3', 'x_pose': '1.0', 'y_pose': '13.7321', 'z_pose': '0.01','model': 'burger'},
    ...
    model_paths = {
        'waffle': {
            'urdf': os.path.join(get_package_share_directory('turtlebot3_multi_robot'), 'urdf', 'turtlebot3_waffle.urdf'),
            'sdf': os.path.join(get_package_share_directory('turtlebot3_multi_robot'), 'models', 'turtlebot3_waffle', 'model.sdf')
        },
        'burger': {
            'urdf': os.path.join(get_package_share_directory('turtlebot3_multi_robot'), 'urdf', 'turtlebot3_burger.urdf'),
            'sdf': os.path.join(get_package_share_directory('turtlebot3_multi_robot'), 'models', 'turtlebot3_burger', 'model.sdf')
        },
        # 추가 모델을 여기에 매핑할 수 있습니다.
    }
    world = os.path.join(
        get_package_share_directory('turtlebot3_multi_robot'),
        'worlds', 'hospital_final.world')
    ...
    map_server=Node(package='nav2_map_server',
        ...
        name='map_server',
        output='screen',
        parameters=[{'yaml_filename': os.path.join(get_package_share_directory('turtlebot3_multi_robot'), 'worlds', 'tb2_map.yaml'),
                     },],
    ...
    for robot in robots:
        ...
        if model_type not in model_paths:
            raise ValueError(f"Model type '{model_type}' not defined in model_paths.")
        urdf = model_paths[model_type]['urdf']
        sdf = model_paths[model_type]['sdf']
    ...
    for robot in robots:
        ...
        messages = {
            'tb1' : '{header: {frame_id: map}, pose: {pose: {position: {x: -8.22167, y: 10.2748, z: 0.0}, orientation: {x: 0.0, y: 0.0, z: -0.4416, w: 0.897212}}, }}',
            'tb2' : '{header: {frame_id: map}, pose: {pose: {position: {x: 0.0229327, y: -0.769658, z: 0.0}, orientation: {x: 0.0, y: 0.0, z: 0.907896, w: 0.419197}}, }}'
        }
        message = messages[robot['name']]  
